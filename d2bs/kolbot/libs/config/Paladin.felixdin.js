...
function LoadConfig() {
	...
	Scripts.AutoSmurf = false;
		...
		Config.AutoSmurf.TeleportingSorc = "felixdin"; // your leader sorceress charname.
		Config.AutoSmurf.BoBarb = ""; // boBarb charname
		Config.AutoSmurf.OtherChars = []; // all team charnames, excluding the leader and boBarb
		Config.AutoSmurf.AllTeamProfiles = ["smiter"]; // the whole team PROFILE names
	...
	Scripts.BattleOrders = false;
		...
		Config.BattleOrders.Getters = ["felixia"]; // List of players to wait for before casting Battle Orders (mode 0). All players must be in the same area as the BOer.
	...
	if (doingBaal) {
		Scripts.Baal = true;
			Config.Baal.HotTPMessage = "a5";
			Config.Baal.SafeTPMessage = "1";
	...
	*/
	Config.Leader = "felixdin"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	// Config.QuitList = ["felixia"]; // List of character names to quit with. Example: //Config.QuitList = ["MySorc", "MyDin"];
	Config.QuitListMode = 0; // 0 = use character names; 1 = use profile names (all profiles must run on the same computer).
	Config.QuitListDelay = [7, 9]; // Quit the game with random delay in case of using Config.QuitList. Example: Config.QuitListDelay = [1, 10]; will exit with random delay between 1 and 10 seconds.
	...
	Scripts.AutoBaal = false; // Baal leecher with auto leader assignment
		Config.AutoBaal.FindShrine = true; // false = disabled, 1 = search after hot tp message, 2 = search as soon as leader is found
	...
	Scripts.Rushee = false; // Automatic rushee, works with Rusher. Set Rusher's character name as Config.Leader
		...
		Config.Rushee.Bumper = true; // Do Ancients and Baal. Minimum levels: 20 - norm, 40 - nightmare
	...
	Scripts.ShopBot = false; // Shopbot script. Automatically uses shopbot.nip and ignores other pickits.
		...
		// Put item classid numbers ohr names to scan (remember to put quotes around names). Leave blank to scan ALL items. See libs/config/templates/ShopBot.txt
	...
	Scripts.ClearAnyArea = false; // Clear any area. Uses Config.ClearType to determine which type of monsters to kill.
		Config.ClearAnyArea.AreaList = [
			74, // "Arcane Sanctuary",
		]; // List of area ids to clear. See sdk/areas.txt
	...
	Config.AutoMule.Exclude = [
		...
		"[name] == mageplate # [ias] == 45", //treachery
		];
	 ...
	 */
	Config.Recipes.push([Recipe.Socket.Weapon, "Phase Blade"]); // Socket pb
	...
	Config.Greetings = ["Hello $name, thanks for joining in the suckage."]; // Example: ["Hello, $name (level $level $class)"]
	Config.DeathMessages = ["Watch out for those damned $killer, hang in there $name!"]; // Example: ["Watch out for that $killer, $name!"]
	Config.Congratulations = ["ding ding GRATZ ON THE LVL DUDE!!!"]; // Example: ["Congrats on level $level, $name!"]
	...
	// Config.AttackSkill[3] = 106; // Primary skill to others. ZEAL--TODO
	...
	Config.Redemption = [50, 0]; // Switch to Redemption after clearing an area if under designated life or mana. Format: [lifepercent, manapercent]
