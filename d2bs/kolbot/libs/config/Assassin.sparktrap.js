...
function LoadConfig() {
	...
	Scripts.AutoSmurf = false;
		...
		Config.AutoSmurf.TeleportingSorc = "felixia"; // your leader sorceress charname.
		Config.AutoSmurf.BoBarb = ""; // boBarb charname
		Config.AutoSmurf.OtherChars = ["sparktrap"]; // all team charnames, excluding the leader and boBarb
		Config.AutoSmurf.AllTeamProfiles = ["USWestSorc", "sparktrap"]; // the whole team PROFILE names
	...
	*/
	...
	Config.QuitList = ["felixdin"]; // List of character names to quit with. Example: //Config.QuitList = ["MySorc", "MyDin"];
	Config.QuitListMode = 0; // 0 = use character names; 1 = use profile names (all profiles must run on the same computer).
	Config.QuitListDelay = [8, 10]; // Quit the game with random delay in case of using Config.QuitList. Example: Config.QuitListDelay = [1, 10]; will exit with random delay between 1 and 10 seconds.
	...
	Scripts.DiabloHelper = false; // Chaos helper, kills monsters and doesn't open seals on its own.
		Config.DiabloHelper.Wait = 180; // Seconds to wait for a runner to be in Chaos. If Config.Leader is set, it will wait only for the leader.
	...
	Scripts.BaalHelper = false;
		Config.BaalHelper.Wait = 180; // Seconds to wait for a runner to be in Throne
	...
	//-----special--------
	// Config.PickitFiles.push("autosmurf/assassin.xpac.nip");
	...
	Config.GambleItems.push(179); // assassincaw
	Config.GambleItems.push("Claws"); // assassincaw
	Config.GambleItems.push("Claws");
	Config.GambleItems.push("Assassin Claw"); // assassincaw
	// Config.GambleItems.push(""); // assassincaw
	...
	Config.AttackSkill[2] = 253; // Primary untimed skill to bosses. Keep at -1 if Config.AttackSkill[1] is untimed skill.
	Config.AttackSkill[3] = 273; // Primary skill to others.
	Config.AttackSkill[4] = 253; // Primary untimed skill to others. Keep at -1 if Config.AttackSkill[3] is untimed skill.
	...
	Config.AutoBuild.Template = "Trapsin";	//	The name of the build associated with an existing 
