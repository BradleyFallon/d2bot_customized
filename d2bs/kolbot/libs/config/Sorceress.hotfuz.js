...
function LoadConfig() {
	...
	Scripts.AutoSmurf = false;
		...
		Config.AutoSmurf.TeleportingSorc = "hotfuz"; // your leader sorceress charname.
		Config.AutoSmurf.BoBarb = "skwizbo"; // boBarb charname
		Config.AutoSmurf.OtherChars = ["Skwiz"]; // all team charnames, excluding the leader and boBarb
		Config.AutoSmurf.AllTeamProfiles = ["USWestSorc", "skelly", "boer"]; // the whole team PROFILE names
	...
	// Team MF system
	Config.MFLeader = false; //true; // Set to true if you have one or more MFHelpers. Opens TP and gives commands when doing normal MF runs.
	...
	Scripts.Treehead = false; //true;
	Scripts.Smith = false; //true;
	Scripts.BoneAsh = false; //true;
	Scripts.Countess = true; //true;
		Config.Countess.KillGhosts = true;
	Scripts.Andariel = true; //true;
	Scripts.Cows = false;

	// *** act 2 ***
	Scripts.Radament = false; //true;;
	...
	Scripts.Bonesaw = false; //true;;
		Config.Bonesaw.ClearDrifterCavern = false;
	Scripts.Snapchip = false; //true;;
	...
	*/
	...
	Config.Leader = "hotfuz"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	//Config.QuitList = ["skwizbo", "Skwiz"]; // List of character names to quit with. Example: //Config.QuitList = ["MySorc", "MyDin"];
	...
	Scripts.Rusher = false; // Rush bot. For a list of commands, see Rusher.js
		Config.Rusher.WaitPlayerCount = 2; // Wait until game has a certain number of players (0 - don't wait, 8 - wait for full game).
	...
	// Config.PickitFiles.push("early.nip");
	...
	//-----special--------
	// Config.PickitFiles.push("autosmurf/sorceressfire.xpac.nip");
	...
	Config.GambleItems.push(175);
	Config.GambleItems.push(176);
	Config.GambleItems.push(177);
	Config.GambleItems.push(178);
	Config.GambleItems.push(179);
	Config.GambleItems.push(180);
	Config.GambleItems.push(181);
	Config.GambleItems.push(182);
	Config.GambleItems.push(183);
	Config.GambleItems.push(184);
	...
	Config.Recipes.push([Recipe.Rune, "Fal Rune"]); // Upgrade Pul to Um
	...
	Config.Greetings = ["Hi $name"]; // Example: ["Hello, $name (level $level $class)"]
	Config.DeathMessages = ["lul noobs"]; // Example: ["Watch out for that $killer, $name!"]
	Config.Congratulations = ["g"]; // Example: ["Congrats on level $level, $name!"]
	...
	// Config.AttackSkill[3] = 47; // Primary skill to others.
	// Config.AttackSkill[4] = 56; // Primary untimed skill to others. Keep at -1 if Config.AttackSkill[3] is untimed skill.
	...
	Config.StaticList = ["Griswold", /*"Gargoyle Trap", */"Andariel", "Duriel", "Izual", "Diablo", "Baal", "Mephisto", "Venom Lord", "Minon Of Destruction", "4c", "Korlic", "Talic"]; // List of monster NAMES or CLASSIDS to static. Example: Config.StaticList = ["Andariel", 243];
	...
	Config.AutoStat.Build = [
		...
		["strength", 45], // get up to 45 strength (level 20)
