...
//STUFF
	...
	this.hireMerc = function(another) {
		print("hiring merc");
		print("I want to do this manually. Dont want to lose gear etc.");
		// return false
...
//QUESTS
	this.den = function () {
		...
		if (true) { // Haven't cleared the Den yet.
			this.teamInGame();

			if (false) { // All characters grab Cold Plains Waypoint in Normal. Only the Teleporting Sorc grabs it in Nightmare and Hell.
...
//MAIN
	...
	*/
	...
	if (!me.getQuest(7, 0) || !this.partyLevel(tristLvl)) { // Andariel is not done.
		Town.goToTown(1);

		//den
		if (!this.partyLevel(caveLvl) ){ //!me.getQuest(1, 0)) {
			this.den();
		}
		// this.den();

		if (true) { // Normal difficulty.
			//cave
			if (!this.partyLevel(caveLvl)) { // Haven't killed Blood Raven, have completed the Den of Evil and the party hasn't reached the caveLvl requirement.
				this.cave();
			}
			//blood raven
			// !me.getQuest(2, 0) && me.getQuest(1, 0) && this.partyLevel(caveLvl)
			if (false) { // Haven't killed Blood Raven, have completed the Den of Evil and the party has reached the caveLvl requirement.
				// this.blood();
			}

			//cain
			if (false) { // Haven't completed The Search for Cain and the party has reached the caveLvl requirement.
			...
			//trist
			if (this.partyLevel(caveLvl) && !this.partyLevel(tristLvl)) { // Have completed The Search for Cain and the party hasn't reached the tristLvl requirement
	...
	//act2
	if ((this.partyLevel(tristLvl) && !this.partyLevel(tombsLvl))) {
		...
		if (teleportingSorc) { // I am the leader.
			...
			while (!this.partyLevel(tombsLvl)) { // Haven't completed Duriel.
				if (cube) {
					if (true)
						this.hireMerc();
					this.cube();
	...
	//act3
	if (this.partyLevel(tombsLvl) && me.getQuest(15, 0)) { // "Able to go to Act IV" (AKA haven't gone thru red portal to Act 4) is not done and Duriel is.
		Town.goToTown(3);
		if (teleportingSorc ) { // I am the Teleporting Sorc
	...
	if (this.partyLevel(mephLvl) && !this.partyLevel(diaLvl)) {
	...
	//act5
	if (this.partyLevel(diaLvl) && me.gametype === 1 && teleportingSorc && me.getQuest(39, 0) && !getWaypoint(38) )
		Pather.getWP(129, false);
	if (this.partyLevel(diaLvl) && me.gametype === 1 && me.getQuest(39, 0) && ((me.diff == 1 && !this.partyLevel(baalLvlnm)) || me.diff == 2)) { //Dark-f
		if (teleportingSorc) {
	...
	if (false && me.gametype === 1 && me.getQuest(28, 0)) { //Dark-f
		Town.goToTown(5);
