...
function doLog(msg) {
    print(msg);
    say(msg); // Skwiz wants to see logging in normal chat during autosmurf
};
...
function AutoSmurf() {
	...
	var caveLvl = 7,
		...
		tombsLvl = 25,
		mephLvl = 26,
		diaLvl = 28, // Getting about 15% per run at 27
		baalLvl = 48,
	...
	this.checkRole = function () { // Checks Config settings to determine role.
		if (Config.AutoSmurf.TeleportingSorc === me.name) {
			...
			if (!otherChar) {
				doLog("I am not assigned a role in my Config file. Please rectify this omission and restart."); // SiC-666 TODO: Make this red text or throw an error instead.
    ...
    this.readyCheck = function () {
		...
		if ( iAmReady && Config.AutoSmurf.TeamSize === 1 ) {
			...
			doLog("Team is ready! Telling others :)");
    ...
    this.updateProgress = function () {
        Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "lowestDiff:" + me.diff);

        if (!me.getQuest(1, 0)) {
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doDen");
        };
        if (!me.getQuest(2, 0) && me.getQuest(1, 0) && !this.partyLevel(caveLvl)) { // Haven't killed Blood Raven, have completed the Den of Evil and the party hasn't reached the caveLvl requirement.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doCave");
        }
        // This condition should work for all difficulties. Must be caveLvl to reach Nightmare & Hell difficulty.
        // Not super necessary in Nightmare & Hell difficulty, but might as well...
        if (!me.getQuest(2, 0) && me.getQuest(1, 0) && this.partyLevel(caveLvl)) { // Haven't killed Blood Raven, have completed the Den of Evil and the party has reached the caveLvl requirement.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doBlood");
        }
        // This condition should work for all difficulties. Must be caveLvl to reach Nightmare & Hell difficulty.
        if (!me.getQuest(4, 0) && this.partyLevel(caveLvl)) { // Haven't completed The Search for Cain and the party has reached the caveLvl requirement.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doCain");
        }
        if (me.getQuest(4, 0) && !this.partyLevel(tristLvl)) { // Have completed The Search for Cain and the party hasn't reached the tristLvl requirement
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doTrist");
        }
        if (!me.getQuest(7, 0) && ((me.diff === 0 && this.partyLevel(tristLvl)) || (me.diff !== 0))) {
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doAndy");
        }
        if (!me.getQuest(17, 0)) { // Haven't completed Lam Esen's Tome.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doLamEsen");
        }
        if (!me.getItem(553) && !me.getItem(174) && !me.getQuest(18, 0)) { // Don't have Eye and don't have Khalim's Will and haven't completed Khalim's Will.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doEye");
        }
        if (!me.getItem(554) && !me.getItem(174) && !me.getQuest(18, 0)) { // Don't have Heart and don't have Khalim's Will and haven't completed Khalim's Will.
        	Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doHeart");
        }
        if (!me.getItem(555) && !me.getItem(174) && !me.getQuest(18, 0)) { // Don't have Brain and don't have Khalim's Will and haven't completed Khalim's Will.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doBrain");
        }
        if (me.getItem(174) || (me.getItem(553) && me.getItem(554) && me.getItem(555)) || !me.getQuest(20, 0) || !me.getQuest(21, 0)) { // Have Khalim's Will or have Eye, Heart, and Brain, or Golden Bird isn't complete, or The Blackened Temple isn't complete.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doTrav");
        }
        if (!me.getQuest(23, 0) && me.getQuest(18, 0) && me.getQuest(21, 0) ) { //no matter Golden bird && me.getQuest(20, 0)) { // Haven't been "Able to go to Act IV" yet and have completed Khalim's Will (AKA the stairs to Durance of Hate Level 1 are open), The Blackened Temple (AKA everyone can enter a Durance Of Hate Level 3 Town Portal), and Golden Bird.
            Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doMephisto");
		}
		if (!me.getQuest(25, 0) || me.getQuest(25, 1)) {
			Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doIzual");
		};
		if (!me.getQuest(35, 1) && !me.getQuest(35, 0)) {
			Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doShenk");
		};
		if ((!me.getQuest(36,0) || me.getQuest(36,1)) && me.diff < 2) {
			Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doBarbs");
		};
		// if (!me.getQuest(37, 0)){ // SOMETHING IS WRONG WITH THIS ONE. my bobarb is going straight to ancients but sorc is teleporting to anya.
		// 	Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doAnya");
		// };
		if (!me.getQuest(39, 0)){
			Messaging.sendToList(Config.AutoSmurf.AllTeamProfiles, "doAncients");
		};
		doLog("Sharing my progress in copydata...");
		delay(2000);
    };
	...
	this.start = function () {
		...
		//misc : starting & teams
		var startSharing = this.partyLevel(6); // Doesn't need to be a global variable.
        ...
        doLog("startSharing: " + startSharing + " lowestAct: " + lowestAct);

        this.updateProgress();

		if (me.findItem(546) || me.findItem(547) || potion ) { // Have A Jade Figurine, The Golden Bird or the Potion of Life or need to Return to Alkor for Reward. Tell the Teleporting Sorc so she gets us to process it.
	...
	//	doLog("myWaypoints length: " + myWaypoints.length); // Should be 34 in Expansion and 26 in Classic.
	...
	//---------------------Process Give/Receive Lists---------------------
		doLog("Start sharing Waypoints.");

		for (i in waypointsToShare) {
			doLog("Current waypointsToShare element: " + i);

			while (waypointsToReceive.length) {
				doLog("waypointsToReceive: " + waypointsToReceive[0]);
			...
			if (!portal) { // Don't share this Waypoint if someone else is already.
				...
				doLog("Sharing a Waypoint in area: " + waypointAreas[i]);

				doLog("playerList.length: " + playerList.length + " finishedPlayerCount: " + finishedPlayerCount);
				...
				for (var j = 0 ; j <100 ; j += 1) { //Here is some wrong, it is too long time to wait. Dark-f < 720 ; j += 1) { // Wait up to 3 minutes Team Members to grab the Waypoint.
					...
					do {
						if (player.name !== me.name) {
							//doLog("player name is" + player.name);
		...
		doLog("Done sharing Waypoints. Start receiving Waypoints.");

		doLog("waypointsToReceive.length: " + waypointsToReceive.length);
		...
		doLog("Done receiving Waypoints.");
	...
	// 			if (!teamIsReady) { // Only need to change teamIsReady to true once.
	// 				teamIsReady = true;
	// 				doLog("Received team is ready!");
	// 			}
	// 			break;
	// 		case "at waypoint":
	// 			atWPCount += 1;
	// 			doLog("atWPCount = " + atWPCount);
	// 			break;
	// 		case "ready to drink":
	// 			readyToDrink += 1;
	// 			doLog("readyToDrink = " + readyToDrink);
	// 			break;
	// 		case "cube":
	// 			cube = true;
	// 			break;
	...
	// 			doLog("readyToKillDiablo = " + readyToKillDiablo);
	...
	this.waitForPartyMembers = function (area) {
		...
		doLog("Waiting for Party Members.");
	...
	this.waitForPartyMembersByWaypoint = function () {
		...
		doLog("Waiting for Party Members by Waypoint.");
	...
	this.giveGold = function () {
		...
		doLog("Dropping " + Math.round(dropAmmount) + " Gold.");
...
*/
	...
	this.receiveWaypoint = function (index) {
		...
		doLog("Receiving Waypoint in area: " + waypointAreas[index]);
	...
	this.giveBo = function () {
        ...
        // I want this to work if BOer is ahead in quests.
        // Simplest solution for now is to always go to the safest option
		if (lowestAct < 6) {
			Town.goToTown(1);
	...
	this.clearToExit = function (currentarea, targetarea, cleartype) { // SiC-666 TODO: add moving to exit without clearing after XX minutes.
		doLog("Start clearToExit");

		doLog("Currently in: " + Pather.getAreaName(me.area));
		doLog("Currentarea arg: " + Pather.getAreaName(me.area));

		delay(250);
		doLog("Clearing to: " + Pather.getAreaName(targetarea));
	...
	this.travel = function (goal) { // 0->9, a custom waypoint getter function
		...
		//doLog("Traveling to " + getArea(destination).name);
		...
		doLog("Travel course = " + target.course);
		...
		if (nextAreaIndex < areaIDs.length) { // If next area index is invalid, return true.
			...
			for (nextAreaIndex ; nextAreaIndex < areaIDs.length; nextAreaIndex += 1) {
				doLog("nextAreaIndex = " + nextAreaIndex);
				doLog("Next location name = " + getArea(areaIDs[nextAreaIndex]).name);
				...
				case 129: // The Worldstone Keep Level 2
					try{
						...
						doLog("Attempting to use any portal");
				...
				if (wpAreas.indexOf(areaIDs[nextAreaIndex]) > -1) { // Check if the next area (which we are now in) has a waypoint. If it does, grab it, go to town, wait for party, run chores, take waypoint, and wait for the party.
					...
					if (me.diff === 0 && (goal === 0 || goal === 1 || goal === 2 || goal === 3) && areaIDs[nextAreaIndex] != destination) { // Don't wait for team if the destination has been reached. (all desinations have a waypoint)
						doLog("start this.travel() waypoint wait");
						...
						doLog("end this.travel() waypoint wait");
					}
				}
			}
...
//STUFF
		...
		if (item) {
			do {
				if (!Item.canEquip(item)) {
					doLog("Removing an item I can no longer wear: " + item.name + ".");
			...
			if (item) {
				if (Storage.Inventory.CanFit(item)) {
					...
					if (Pickit.canMakeRoom()) {
						doLog("ÿc7Trying to make room for " + Pickit.itemColor(item) + item.name);

						Town.visitTown(); // Go to Town and do chores. Will throw an error if it fails to return from Town.
					} else {
						doLog("ÿc7Not enough room for " + Pickit.itemColor(item) + item.name);
	...
	this.cubeFlail = function () {
		...
		if (!eye || !heart || !brain || !flail) {
			doLog("cubeFlail failed: missing ingredient(s)");
		...
		if (me.itemoncursor) { // Seems like Item.equip() doesn't want to keep whatever the sorc has for a weapon, so lets put it into inventory without checking it against Pickit.
			...
			if (cursorItem) {
				if (Storage.Inventory.CanFit(cursorItem)) {
					doLog("Keeping weapon by force.")

					Storage.Inventory.MoveTo(cursorItem);
				} else {
					me.cancel();
					doLog("No room to keep weapon by force.");
	...
	this.placeFlail = function () { // SiC-666 TODO: Rename this function to smashOrb, check orb.mode in the loop and stop when it has been smashed.
		...
		doLog("Smashing the Compelling Orb.");
	...
	this.tombs = function() { // Teleporting Sorc shares Canyon of The Magi waypoint with the others and they each clear to the chest in all of the tombs.
		...
		if (teleportingSorc) {
			...
			if (!getWaypoint(17)) { // Canyon Of The Magi
				...
				doLog("Waiting for Canyon Of The Magi TP.");
		...
		if (!this.partyLevel(tombsLvl)) {
			doLog("Not ready to start Duriel.");
...
*/
		...
		if (potion) {
			doLog("Drinking the Figurine potion!");
		...
		doLog("Seriously, the 'Potion of Life'?? Tastes more like 'Potion of Piss'!!!");
	...
	this.diablo = function () {
		...
		this.vizierSeal = function () {
			doLog("Viz layout " + this.vizLayout);
		...
		this.seisSeal = function () {
			doLog("Seis layout " + this.seisLayout);
		...
		this.infectorSeal = function () {
			doLog("Inf layout " + this.infLayout);
...
*/
		...
		if (me.y > 5400) {
			doLog("Started at Entrance.");

			this.followPath(this.entranceToStar);

			Attack.clear(30, 0, false, this.sort);
		} else {
			doLog("Started at Star.");
		}
		this.Bo();
		this.vizierSeal();
		this.Bo();
		...
		if (me.gametype === 0 && (!this.partyLevel(diaLvl) || (me.diff === 1 && !this.partyLevel(diaLvlnm)) || me.diff === 2)) { // Don't kill Diablo in classic unless the difficulty specific level requirement is met.
			doLog("Not ready to kill diablo!!"); // NOTE TO USERS: In classic, never kill Diablo in Hell difficulty with this script. It's better to switch to a CS Taxi script that will run faster and maintain Diablo virgin kills.
	...
	//(<3 Larryw)
		...
		if (!me.getQuest(36,1) && teleportingSorc) {
			...
			for ( k = 0  ; k < coords.length ; k += 1) {
				doLog("going to barbspot "+(k+1)+"/"+barbSpots.length);
	...
	addEventListener("copydata", (id, data) => {
		...
		if (id == 55) {
			...
			case "team is ready":
                ...
                break;
            case (msg.match("lowestDiff:") ? msg : null): // Check if sharing lowest act
                var diff = msg.split(":")[1]
                doLog("diff is: " + diff)
                if (diff < me.diff) {
                    lowestDiff = diff;
                } else {
	...
	*/
	...
	while (!this.teamInGame(true)) { // Wait for the AutoSmurf Team to join.
		...
		if (getTickCount() - tick > 5 * 60 * 1000) { // Leave the game after 5 minutes of waiting.
			doLog("Team wasn't in game within 5 minutes.");
    ...
    doLog("Enough prep... Time to start our adventure! Starting act: " + lowestAct);
    doLog("Lowest difficulty in party: " + lowestDiff);

	//act1
    if (lowestAct === 1 || doCain) {
        doLog("In act 1 adventure guide.")
        Town.goToTown(1);
        ...
        //cave
        if (doCave) { // Haven't killed Blood Raven, have completed the Den of Evil and the party hasn't reached the caveLvl requirement.
            this.cave();
            this.updateProgress();
        }
        //blood raven
        if (doBlood) { // Haven't killed Blood Raven, have completed the Den of Evil and the party has reached the caveLvl requirement.
            this.blood();
            this.updateProgress();
        }

        //cain
        if (doCain) { // Haven't completed The Search for Cain and the party has reached the caveLvl requirement.
        ...
        //trist
        if (doTrist) { // Have completed The Search for Cain and the party hasn't reached the tristLvl requirement
            this.trist();
	...
	//act2
	if (lowestAct === 2) { // Duriel is not done and Andariel is.
		Town.goToTown(2);
		...
		if (teleportingSorc) { // I am the leader.
			...
			while (lowestAct === 2) { // Haven't completed Duriel.
				if (cube) {
					// if (me.diff === 0)
					// 	this.hireMerc();
					this.cube();
	...
	//act3
	if (lowestAct === 3) { // "Able to go to Act IV" (AKA haven't gone thru red portal to Act 4) is not done and Duriel is.
		Town.goToTown(3);
		if (teleportingSorc ) { // I am the Teleporting Sorc
			...
			if (doLamEsen) { // Haven't completed Lam Esen's Tome.

				this.lamEsen();
                this.updateProgress();
			}

			if (doEye) { // Don't have Eye and don't have Khalim's Will and haven't completed Khalim's Will.
				this.eye();
                this.updateProgress();
			}

			if (doHeart) { // Don't have Heart and don't have Khalim's Will and haven't completed Khalim's Will.
				this.heart();
                this.updateProgress();
			}

			if (doBrain) { // Don't have Brain and don't have Khalim's Will and haven't completed Khalim's Will.
				this.brain();
                this.updateProgress();
			}

			if (doTrav) { // Have Khalim's Will or have Eye, Heart, and Brain, or Golden Bird isn't complete, or The Blackened Temple isn't complete.
			...
			if (doMephisto) { //no matter Golden bird && me.getQuest(20, 0)) { // Haven't been "Able to go to Act IV" yet and have completed Khalim's Will (AKA the stairs to Durance of Hate Level 1 are open), The Blackened Temple (AKA everyone can enter a Durance Of Hate Level 3 Town Portal), and Golden Bird.
			...
			while (lowestAct === 3) { // Haven't completed "Able to go to Act IV" (AKA haven't gone thru red portal to Act 4)
				if (LamEssen) {
					this.lamEsen();
					LamEssen = false;
				}
	...
	if (lowestAct === 4 || (lowestAct === 5 && !this.partyLevel(diaLvl))) { // If in act 5 but not diablo level, still run diablo
		Town.goToTown(4);
		...
		if (doIzual) { // The Fallen Angel is not done or it has been started.
			this.izual();
			this.updateProgress();
		}
	...
	//act5
	// This is extra extra and I dont want to worry until the basic questing and leveling is perfect

	// if (lowestAct === 5 && me.gametype === 1 && teleportingSorc && me.getQuest(39, 0) && !getWaypoint(38) ) // ancients done and not 
	// 	Pather.getWP(129, false);
	// if (lowestAct === 5 && me.gametype === 1 && me.getQuest(39, 0) && ((me.diff == 1 && !this.partyLevel(baalLvlnm)) || me.diff == 2)) { //Dark-f
	// 	if (teleportingSorc) {
	...
	if ((lowestAct === 5 && this.partyLevel(diaLvl))) { //Dark-f
		Town.goToTown(5);
...
var OnOff = {
	...
	find: function() {
			...
			for (n in CustomConfig) {
				if (CustomConfig.hasOwnProperty(n)) {
					if (CustomConfig[n].indexOf(me.profile) > -1) {
						if (notify) {
							doLog("ÿc2Loading custom config: ÿc9" + n + ".js");
						}
		...
		doLog("ÿc;ConfigChanger ÿc0:: Couldn't find config file.");
		return false;
	},
	...
	disable: function(line) {
		...
		for (var i = 0; i < lines.length; i++) {
			if (lines[i].match(line + " = ")) {
				...
				doLog("ÿc;ConfigChanger ÿc0:: Disabled: " + line);
	...
	enable: function(line) {
		...
		for (var i = 0; i < lines.length; i++) {
			if (lines[i].match(line + " = ")) {
				...
				doLog("ÿc;ConfigChanger ÿc0:: Enabled: " + line);
				Misc.fileAction(this.file, 1, lines.join("\n"));
